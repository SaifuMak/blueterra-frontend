...
export default function EditItinerary() {
    ...
    const inputStyle = 'placeholder:text-[#949393] w-full h-fit bg-white rounded-[4px] border border-[#B5B5B5] outline-none  py-2 px-4'
    ...
    const [gallery, setGallery] = useState([{ id: '', image: '', title: '', image_public_url: '', is_checked: false }]);
        ...
        <div className={`h-screen w-full ${rubik.className}`}>
            ...
            >
                <div className=" w-full h-full  flex">
                    ...
                    {isLoading ? (
                        <div className="  flex-1 h-[97vh] flex-center  bg-slate-50">
                        ...
                        <div className=" flex-1 relative    ml-4  mr-8  p-10 rounded-xl bg-admin-light-dark-background w-full flex justify-center overflow-y-scroll h-[97vh] z-50">
                            ...
                            <div className=" flex space-x-3 max-xl:text-sm text-white  absolute   right-5 top-5">
                                <button type="submit" formNoValidate name="action"
                                    value="draft" className=" bg-[#524D4D] cursor-pointer  w-32 h-fit  py-2 flex-center  rounded-sm">{isLoading ? <LoaderIcon className='animate-spin text-2xl ' /> : 'Save to Draft'}</button>
