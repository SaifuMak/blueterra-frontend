...
function HotelsSection({ hotels, setHotels, textAreaStyle, inputStyle, handleReorder }) {
    ...
    const handleAddHotel = () => {
        ...
        setHotels([...hotels, { title: '', description: '', image: null, coordinates: '', location: '', mapLink: '', rating: 5 }])
        setTimeout(() => {
    ...
    return (
        <div className="flex flex-col w-9/12 xl:w-6/12  transition-all duration-300 ease-in-out">
            ...
            <div className="  flex items-center space-x-8">
                <h2 className=" text-xl text-dark-blue font-medium">Hotels</h2>
            ...
            {hotels?.map((hotel, index) => (
                <div key={index} ref={(el) => (hotelsRefs.current[index] = el)} className={`  my-2  relative flex flex-col space-y-4`}>
                    ...
                    <p className=" font-medium mt-4 text-lg  ">Hotel {index + 1}</p>
                    <input type="text" value={hotel.title} onChange={(e) => handleHotelChange(index, 'title', e.target.value)} placeholder="Hotel name" className={`${inputStyle}`} required />
                    <input type="text" value={hotel.location} onChange={(e) => handleHotelChange(index, 'location', e.target.value)} placeholder="Location" className={`${inputStyle}`} required />
                    <div className=" flex items-center ">
                        <p className=" font-normal">Ratings</p>
                        <div className=" flex space-x-0.5 ml-4">
                            {[...Array(5)].map((_, ratingIndex) => (
                                <IoIosStar onClick={() => handleHotelChange(index, 'rating', ratingIndex + 1)} key={ratingIndex} className={` ${ratingIndex + 1 <= hotel.rating ? 'fill-[#FFCB1F]' : 'fill-slate-400'} size-4 cursor-pointer`} />
                            ))}
                        </div>
                    </div>
                    <textarea name="" id="" placeholder="Description" value={hotel.description} onChange={(e) => handleHotelChange(index, 'description', e.target.value)} className={`${textAreaStyle} w-full mt-4`} required></textarea>
                    ...
                    <div className=" flex w-full space-x-8 mt-4  ">
                        <ImageUploader
                            ...
                            selectedFile={hotel.image}
                            setSelectedFile={(file) => handleHotelChange(index, "image", file)}
